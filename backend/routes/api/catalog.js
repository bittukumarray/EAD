...
router.get("/elastic/name/:crop", async (req, res, next) => {
  const crop = req.params.crop;
  try {
    const data = await Crops.find(
      { name: { $regex: crop, $options: "i" } },
...
router.get("/elastic/city/:city", async (req, res, next) => {
  const city = req.params.city;
  try {
    const data = await Crops.find(
      { city: { $regex: city, $options: "i" } },
...
router.get("/elastic/anydata/:data", async (req, res, next) => {
  const reqData = req.params.data;
  try {
    const data = await Crops.find(
      {
...
router.get("/elastic/all", async (req, res, next) => {
  try {
    const data = await Crops.find({}, null, { sort: "-date" }, function (
      err,
      docs
