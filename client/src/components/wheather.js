...
class WheatherPage extends Component {
  ...
  <div class="input-group-prepend">
    <button onClick={this.onClickHandler} class="btn btn-md btn-outline-primary m-0 px-3 py-2 z-depth-0 waves-effect" type="button" id="button-addon1">Apply</button>
  </div>
  <input onChange={this.onChangeHandler}  type="text" placeholder="Enter city name" class="form-control" placeholder="" aria-label="Example text with button addon"
    aria-describedby="button-addon1"></input>
</div>
{this.state.data?<div class="card weather-card">

<div class="card-body pb-3">

        <h4 class="card-title font-weight-bold">{this.state.data.name}</h4>
        <p class="card-text">{this.state.data.weather.map((element)=>{
  ...
  <div class="d-flex justify-content-between mb-4">
      <p><i class="fas fa-tint fa-lg text-info pr-2"></i>{this.state.data.clouds.all}% Cloud</p>
    <p><i class="fas fa-leaf fa-lg grey-text pr-2"></i>{this.state.data.wind.speed * 3600/1000} km/h Winds</p>
  </div>
